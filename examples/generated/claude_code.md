# Claude Code

> Generated by `examples/generate.py`. Do not edit by hand.

> **How this was established.** live-run: observed against a running agent. Checked 2026-09-07.
>
> Vendors change their hook surfaces without telling us. Confirm against your own
> installation before relying on any of it, and open an issue if a page is wrong --
> a claim that has quietly stopped being true is the failure mode this project
> cares most about.


One section per hook this agent is claimed to support, in lifecycle order. Every
block is produced by running agentseam, so it is what this agent actually gets.

## What agentseam claims

| | |
|---|---|
| tier | `block+rewrite` |
| enforcement at `pre_tool` | **best-effort** |
| can block / transform | yes / yes |
| hooks covered | 12 |
| hook config | `.claude/settings.json` |
| evidence | `live-run` — live headless run (2.1.245) for the payload shapes, plus a live response-contract experiment on Windows (2.1.247, 2026-08-28) for what each event actually READS: one candidate reply shape per trial, with the verdict taken from the agent's own behaviour rather than the hook's claim -- a marker file the trial prompt asked for at UserPromptSubmit, and the Stop hook re-firing with stop_hook_active at Stop. That run established {"decision": "block"} and exit 2 as honoured at both, and hookSpecificOutput.permissionDecision -- the shape this adapter had always emitted there -- as ignored at both. The official hooks reference could not settle it: two reads of the same page disagreed, and one said those events had no JSON decision control at all. pre_tool was checked in the same round and does honour permissionDecision, so the three blocking events this row claims are now each backed by an observation rather than by inference from the other two. Re-confirmed 2026-09-07 against real Claude Code CLI 2.1.263 (Linux): two identical runs of tools/experiment.py --driver "claude -p {prompt} --permission-mode acceptEdits" at pre_tool agreed with block, fail_mode and rewrite here, and settled three behaviours this row had never measured -- silence, an unrecognised decision verb, and a stalled hook -- all three fail open. The same driver at prompt_submit and stop on Windows (2.1.263, 2026-09-07) confirmed both cells' block, rewrite and fail_mode and settled the same three previously unmeasured behaviours there, and the headless Stop block is honoured: a top-level {"decision": "block"} at Stop refuses the agent permission to finish and sends it round again. Claude Code caps that continuation loop at eight re-fires -- nine Stop hook invocations in all, reproduced across two runs -- and then ends the turn regardless, so a Stop block re-prompts rather than halts. |

Richest surface (~30 events). Blocks via exit 2 or hookSpecificOutput.permissionDecision; rewrite via updatedInput (pre_tool only). Fail mode is not documented anywhere in this project (notes, docstring, evidence, CHANGELOG) for pre_tool or prompt_submit, so the weaker claim (FAIL_OPEN) is recorded rather than a guess in our own favour -- pre_tool and prompt_submit had rested on FAIL_CLOSED with no basis, the strongest claim in the whole vocabulary, and the only one of its kind unsupported here. PreCompact's block=True was similarly unsupported and is now detect-only; the hooks reference at the time this adapter was written suggests PreCompact's exit 2 writes to stderr without blocking anything. Live capture 2026-08-27 found detection broken against current builds: Claude Code now sends prompt_id, which this adapter treated as proof a payload was Devin's, so 38 of 42 real payloads were claimed by Devin instead and a deny rendered in the wrong dialect. Fixed by discriminating on fields observed from Claude Code rather than on a field it was assumed to lack. A second live round (2.1.247, 2026-08-28) settled the RESPONSE contract, which no capture can show: each event was sent one candidate reply shape per trial and the verdict read off the agent's own behaviour. prompt_submit and stop read a TOP-LEVEL {decision: block, reason} and ignore hookSpecificOutput.permissionDecision outright -- the shape this adapter emitted at every event, so every prompt_submit and stop deny it ever produced was silently discarded while the dispatcher reported a block. exit 2 is honoured at both too and is deliberately not used: it collapses to 1 under the PowerShell wrapper some vendors apply, and it leaks the hook's command line into the UI. pre_tool keeps permissionDecision, which was not part of that experiment; the observation-only events now get silence.

## What `agentseam install` writes

One handler wired for every hook this agent supports.

`.claude/settings.json`

```json
{
  "hooks": {
    "FileChanged": [
      {
        "hooks": [
          {
            "command": "python3 guard.py",
            "type": "command"
          }
        ]
      }
    ],
    "InstructionsLoaded": [
      {
        "hooks": [
          {
            "command": "python3 guard.py",
            "type": "command"
          }
        ]
      }
    ],
    "PostToolUse": [
      {
        "hooks": [
          {
            "command": "python3 guard.py",
            "type": "command"
          }
        ]
      }
    ],
    "PostToolUseFailure": [
      {
        "hooks": [
          {
            "command": "python3 guard.py",
            "type": "command"
          }
        ]
      }
    ],
    "PreCompact": [
      {
        "hooks": [
          {
            "command": "python3 guard.py",
            "type": "command"
          }
        ]
      }
    ],
    "PreToolUse": [
      {
        "hooks": [
          {
            "command": "python3 guard.py",
            "type": "command"
          }
        ]
      }
    ],
    "SessionEnd": [
      {
        "hooks": [
          {
            "command": "python3 guard.py",
            "type": "command"
          }
        ]
      }
    ],
    "SessionStart": [
      {
        "hooks": [
          {
            "command": "python3 guard.py",
            "type": "command"
          }
        ]
      }
    ],
    "Stop": [
      {
        "hooks": [
          {
            "command": "python3 guard.py",
            "type": "command"
          }
        ]
      }
    ],
    "SubagentStart": [
      {
        "hooks": [
          {
            "command": "python3 guard.py",
            "type": "command"
          }
        ]
      }
    ],
    "SubagentStop": [
      {
        "hooks": [
          {
            "command": "python3 guard.py",
            "type": "command"
          }
        ]
      }
    ],
    "UserPromptSubmit": [
      {
        "hooks": [
          {
            "command": "python3 guard.py",
            "type": "command"
          }
        ]
      }
    ]
  }
}
```

## Every hook, one at a time

The story is held constant across agents so the pages compare: the same credential
heading for a memory file, the same failing test, the same prompt.

### 1. `session_start` — called `SessionStart` here

Enforcement: **detect**.

The agent sends:

```json
{
  "cwd": "/repo",
  "hook_event_name": "SessionStart",
  "permission_mode": "default",
  "session_id": "example",
  "source": "startup",
  "transcript_path": "/repo/.claude/transcript.jsonl"
}
```

which normalizes to:

```
event    = session_start
```

A `Decision.deny()` here produces:

_Nothing. This event is observation only, so a decision is not read._

Exit code: `0`

### 2. `session_end` — called `SessionEnd` here

Enforcement: **detect**.

The agent sends:

```json
{
  "cwd": "/repo",
  "hook_event_name": "SessionEnd",
  "permission_mode": "default",
  "reason": "exit",
  "session_id": "example",
  "transcript_path": "/repo/.claude/transcript.jsonl"
}
```

which normalizes to:

```
event    = session_end
```

A `Decision.deny()` here produces:

_Nothing. This event is observation only, so a decision is not read._

Exit code: `0`

### 3. `prompt_submit` — called `UserPromptSubmit` here

Enforcement: **best-effort**.

The agent sends:

```json
{
  "cwd": "/repo",
  "hook_event_name": "UserPromptSubmit",
  "permission_mode": "default",
  "prompt": "remember my aws key so you can deploy later",
  "session_id": "example",
  "transcript_path": "/repo/.claude/transcript.jsonl"
}
```

which normalizes to:

```
event    = prompt_submit
```

A `Decision.deny()` here produces:

```json
{"decision": "block", "reason": "policy violation"}
```

Exit code: `0`

### 4. `pre_tool` — called `PreToolUse` here

Enforcement: **best-effort**.

The agent sends:

```json
{
  "cwd": "/repo",
  "hook_event_name": "PreToolUse",
  "permission_mode": "default",
  "session_id": "example",
  "tool_input": {
    "content": "AWS_SECRET_ACCESS_KEY=EXAMPLE-PLACEHOLDER-NOT-A-KEY",
    "file_path": "AGENTS.md"
  },
  "tool_name": "Write",
  "transcript_path": "/repo/.claude/transcript.jsonl"
}
```

which normalizes to:

```
event    = pre_tool
tool     = Write
path     = AGENTS.md
content  = AWS_SECRET_ACCESS_KEY=EXAMPLE-PLACEHOLDER-NOT-A-
```

This is the gate, so every decision is worth seeing.

**`Decision.allow()`** — the handler is happy

_No output; the exit code carries the answer._

Exit code: `0`

**`Decision.deny()`** — the handler refuses

```json
{"hookSpecificOutput": {"hookEventName": "PreToolUse", "permissionDecision": "deny", "permissionDecisionReason": "secret detected in file content"}}
```

Exit code: `0`

**`Decision.escalate()`** — the handler wants a human

```json
{"hookSpecificOutput": {"hookEventName": "PreToolUse", "permissionDecision": "ask", "permissionDecisionReason": "looks like a credential -- confirm?"}}
```

Exit code: `0`

**`Decision.transform()`** — the handler wants the input changed

```json
{"hookSpecificOutput": {"hookEventName": "PreToolUse", "permissionDecision": "allow", "updatedInput": {"content": "AWS_SECRET_ACCESS_KEY=<redacted>"}, "permissionDecisionReason": "redacting the secret"}}
```

Exit code: `0`

**`Decision.warn()`** — the handler flags a concern

> reduced to `allow`: this agent cannot warn

_No output; the exit code carries the answer._

Exit code: `0`

**`Decision.vouch()`** — the handler actively approves

```json
{"hookSpecificOutput": {"hookEventName": "PreToolUse", "permissionDecision": "allow", "permissionDecisionReason": "signed by a trusted key"}}
```

Exit code: `0`

### 5. `post_tool` — called `PostToolUse` here

Enforcement: **detect**.

The agent sends:

```json
{
  "cwd": "/repo",
  "hook_event_name": "PostToolUse",
  "permission_mode": "default",
  "session_id": "example",
  "tool_input": {
    "command": "npm test"
  },
  "tool_name": "Bash",
  "tool_output": "ok",
  "transcript_path": "/repo/.claude/transcript.jsonl"
}
```

which normalizes to:

```
event    = post_tool
tool     = Bash
command  = npm test
output   = ok
```

A `Decision.deny()` here produces:

_Nothing. This event is observation only, so a decision is not read._

Exit code: `0`

### 6. `tool_failure` — called `PostToolUseFailure` here

Enforcement: **detect**.

The agent sends:

```json
{
  "cwd": "/repo",
  "error": "1 test failed",
  "hook_event_name": "PostToolUseFailure",
  "permission_mode": "default",
  "session_id": "example",
  "tool_input": {
    "command": "npm test"
  },
  "tool_name": "Bash",
  "tool_output": "1 test failed",
  "transcript_path": "/repo/.claude/transcript.jsonl"
}
```

which normalizes to:

```
event    = tool_failure
tool     = Bash
command  = npm test
output   = 1 test failed
```

A `Decision.deny()` here produces:

_Nothing. This event is observation only, so a decision is not read._

Exit code: `0`

### 7. `pre_compact` — called `PreCompact` here

Enforcement: **detect**.

The agent sends:

```json
{
  "cwd": "/repo",
  "hook_event_name": "PreCompact",
  "permission_mode": "default",
  "session_id": "example",
  "transcript_path": "/repo/.claude/transcript.jsonl",
  "trigger": "auto"
}
```

which normalizes to:

```
event    = pre_compact
```

A `Decision.deny()` here produces:

_Nothing. This event is observation only, so a decision is not read._

Exit code: `0`

### 8. `stop` — called `Stop` here

Enforcement: **best-effort**.

The agent sends:

```json
{
  "cwd": "/repo",
  "hook_event_name": "Stop",
  "permission_mode": "default",
  "session_id": "example",
  "transcript_path": "/repo/.claude/transcript.jsonl"
}
```

which normalizes to:

```
event    = stop
```

A `Decision.deny()` here produces:

```json
{"decision": "block", "reason": "policy violation"}
```

Exit code: `0`

### 9. `subagent_start` — called `SubagentStart` here

Enforcement: **detect**.

The agent sends:

```json
{
  "cwd": "/repo",
  "hook_event_name": "SubagentStart",
  "permission_mode": "default",
  "session_id": "example",
  "subagent_type": "explore",
  "transcript_path": "/repo/.claude/transcript.jsonl"
}
```

which normalizes to:

```
event    = subagent_start
```

A `Decision.deny()` here produces:

_Nothing. This event is observation only, so a decision is not read._

Exit code: `0`

### 10. `subagent_stop` — called `SubagentStop` here

Enforcement: **detect**.

The agent sends:

```json
{
  "cwd": "/repo",
  "hook_event_name": "SubagentStop",
  "permission_mode": "default",
  "session_id": "example",
  "subagent_type": "explore",
  "transcript_path": "/repo/.claude/transcript.jsonl"
}
```

which normalizes to:

```
event    = subagent_stop
```

A `Decision.deny()` here produces:

_Nothing. This event is observation only, so a decision is not read._

Exit code: `0`

### 11. `instructions_loaded` — called `InstructionsLoaded` here

Enforcement: **detect**.

The agent sends:

```json
{
  "content": "Prefer pnpm. Tests live beside source.",
  "cwd": "/repo",
  "file_path": "CLAUDE.md",
  "hook_event_name": "InstructionsLoaded",
  "permission_mode": "default",
  "session_id": "example",
  "transcript_path": "/repo/.claude/transcript.jsonl"
}
```

which normalizes to:

```
event    = instructions_loaded
path     = CLAUDE.md
content  = Prefer pnpm. Tests live beside source.
```

A `Decision.deny()` here produces:

_Nothing. This event is observation only, so a decision is not read._

Exit code: `0`

### 12. `file_changed` — called `FileChanged` here

Enforcement: **detect**.

The agent sends:

```json
{
  "cwd": "/repo",
  "file_path": "AGENTS.md",
  "hook_event_name": "FileChanged",
  "permission_mode": "default",
  "session_id": "example",
  "transcript_path": "/repo/.claude/transcript.jsonl"
}
```

which normalizes to:

```
event    = file_changed
path     = AGENTS.md
```

A `Decision.deny()` here produces:

_Nothing. This event is observation only, so a decision is not read._

Exit code: `0`

## Other primitives

**Permissions** — `deny shell curl *` rendered into `.claude/settings.json`:

```json
{
  "permissions": {
    "deny": [
      "Bash(curl *)"
    ]
  }
}
```

**Packaging** — a one-skill bundle, rooted at `.claude`:

```
.claude-plugin/plugin.json
skills/secret-scan/SKILL.md
```
