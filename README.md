...
`live-run`, `vendor-source`, `vendor-docs`, `third-party-install` — and most of them are
vendor documentation, which is a claim about what a vendor *says* rather than an observation
of what their build does. Only Claude Code's row rests on a live run. Vendors change hook
