...
event a handler sees, and what `allow` / `deny` / `ask` / `rewrite` / `vouch` each turn into
on the way back, including the ones that get reduced because the agent cannot express them.
