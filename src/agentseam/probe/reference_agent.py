...
def _interpret_g1(body):
    """G1's own decision reading (Stop, UserPromptSubmit): permissionDecision never enters."""
    ...
    # permissionDecision is not read at these gates. Silently allowing is the observed
    # behaviour, and is exactly how an adapter that emits the wrong shape here looks like
    # it is working.
    return ALLOW, "no G1 decision field (permissionDecision is not read here)", None


def _interpret_g2(body):
    """G2's own decision reading (PreToolUse): honours hookSpecificOutput.permissionDecision."""
    ...
    if decision == ASK:
        ...
        # evidence, exactly what this module exists to refuse. See experiment_escalate.py.
...
def _interpret(proc, grammar):
    ...
    return _interpret_g1(body) if grammar == "G1" else _interpret_g2(body)
