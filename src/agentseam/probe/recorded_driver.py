...
def run_trial(agent, trial, *, event, version=None):
    ...
    from . import experiment  # noqa: PLC0415 -- experiment.py imports this module, so this stays lazy

    observed = dict(trial_body["observed"])
    invocations = trial_body["hook_invocations"]
    field, value, reading = experiment._classify(trial, event, observed, invocations)
    return {
        "agent": agent,
        "trial": trial,
        "event": event,
