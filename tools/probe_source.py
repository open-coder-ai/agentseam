...
def render(here, capture_dir):
    ...
    lines = [
        ...
        "    #",
        "    # The RESPONDING adapter is chosen by detecting the payload, not by the argv",
        "    # label -- those are different questions and the label answers the wrong one.",
        "    # Cursor loads Claude Code-format hooks too, confirmed live 2026-08-27: with",
        "    # both .cursor/hooks.json and .claude/settings.json present every event fired",
        "    # TWICE, once per config. So a probe installed as claude_code receives Cursor's",
        "    # own preToolUse payload and used to answer it in Claude Code's dialect, which",
        "    # Cursor does not read -- indistinguishable from the silence two comments up,",
        "    # and therefore a refusal that blocks the command. argv still attributes the",
        "    # RECORD; it just no longer picks the dialect.",
        "    if text is not None and '__unparsed__' not in payload:",
        "        from agentseam import Decision, adapters",
        "        from agentseam.contract import UNKNOWN",
        "",
        "        raw = json.loads(text)",
        "        sender = adapters.detect(raw)",
        "        # detect() declines on a genuine tie (tabnine/gemini_cli share an envelope",
        "        # by design). Falling back to argv is safe only where that adapter actually",
        "        # claims this payload -- otherwise we would be back to answering in a",
        "        # dialect nothing here supports.",
        "        if sender is None:",
        "            argv_mod = adapters.ADAPTERS.get(agent)",
        "            sender = agent if argv_mod is not None and argv_mod.claims(raw) else None",
        "        mod = adapters.ADAPTERS.get(sender) if sender else None",
        "        if mod is not None:",
        "            event = mod.parse(raw)",
        "            # An event we cannot name has no known output contract; inventing one",
        "            # is how a probe starts speaking for a gate it does not understand.",
        "            if event.event is not UNKNOWN:",
        "                out, _code = mod.respond(Decision.allow('agentseam capture probe'), event)",
        "                if out:",
